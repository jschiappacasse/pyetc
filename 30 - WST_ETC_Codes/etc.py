...
from skycalc_cli.skycalc import SkyModel #A tool for simulating the sky background spectrum (e.g., atmospheric emission lines)
from skycalc_cli.skycalc_cli import fixObservatory #Fixes observatory settings for SkyCalc: I guess for the coordinates of the observatory location
...
__all__ = ['ETC', 'asymgauss', 'compute_sky', 'fwhm_asymgauss', 'get_data',
           ...
           'sigma2vdisp', 'update_skytables', 'vdisp2sigma']


class ETC:
    """ Generic class for Exposure Time Computation (ETC) """

    def __init__(self, log=logging.INFO): #constructor, not used in WST
    ...
    def set_logging(self, log): #questo é per modificare nel caso dinamicamente il livello di logging
	...
	# lo prende in fondo al file wst.py
    def _info(self, ins_names, full=False):
        ...
        for ins_name in ins_names:
            ...
            for chan in insfam['channels']:
                ...
                self.logger.info('\t Detector RON %.1f e- Dark %.1f e-/h', ins['ron'],ins['dcurrent']) #e-/pixel/h
                if full:
                    for sky in ins['sky']:
    ...
    def set_obs(self, obs): #un dizionaro a cui posso mettere svariati keys
    ...
    def get_spec(self, ins, dspec, oversamp=10, lsfconv=True):
        ...
        if dspec['type'] == "flatcont":
            ...
            l2 = wave.coord(k) #perche ha solo 2 wave questo spettro
            ...
            kfwhm = dspec.get('kfwhm', 5) #is like setting the default to 5 if not present
            ...
            rspec = sp.subspec(lmin=l1, lmax=l2) #funzione di mpdaf.Spectrum obj to select the spectrum in the range
            # LSF convolution
            if lsfconv:
                spec = rspec.filter(width=ins['lsfpix']*oversamp) #convolution with a box with this range
            else:
                spec = rspec
        else:
    ...
    def get_ima(self, ins, dima, oversamp=10, uneven=1):
        ...
        if dima['type'] == 'moffat':
            ...
            #we use the sersic modified function in order to get the WCS back ! ! !
            ima = sersic(self, ins, dima['reff'], dima['n'], dima.get('ell',0),
    ...
    def truncate_spec_adaptative(self, ins, spec, kfwhm):
        ...
        rspec.data *= spec.data.sum()/rspec.data.sum() #qui normalizziamo la somma a 1
        nl1 = l0 - kfwhm*(l0-l1)
        nl2 = l0 + kfwhm*(l2-l0)
    ...
    def truncate_spec_fixed(self, ins, spec, nsp):
        ...
        rspec.data *= spec.data.sum()/rspec.data.sum() #rinormalizzazione della linea dopo averla ricampionata sulle lunghezze della 'instrans'
...
#        fwhm,_ = ima.fwhm_gauss(center=center, unit_center=None, unit_radius=None)
    ...
    def get_psf_frac_ima(self, ins, flux, spec, lrange=None, oversamp=10, lbin=1):
        ...
        if (ins['type'] == 'IFS') and (obs['ima_aperture_type'] == 'circular_adaptative'):
            ...
            if lrange is None:
                ...
                lrange = [l0 - 0.4*dl, l0 + 0.4*dl] #! ! ! takes the 80% of the spec, why, could be changed maybe?  
            kfwhm_edges = []
            for k in [0,-1]:
    ...
    def snr_from_source(self, ins, flux, ima, spec, loop=False, debug=True):
        ...
        if obs['spec_type'] == 'cont': # flux is in erg/s/cm2/A
            ...
            nspectels = 1
            frac_spec = 1
            size_spec = spec.get_end() - spec.get_start() #this is not used after
            waves = tspec.wave.coord()
        ...
        if (ima is None) :
            ...
            if (obs['ima_type'] == 'sb'):
                ...
                frac_flux = frac_ima*frac_spec #not completely clear to me since they are both one
        ...
        if (not is_ps) or (obs['ima_type'] == 'resolved'):
            ...
            if obs['ima_type'] == 'resolved':
                ...
                self.logger.debug(nspaxels) ### COMMENT AFTER CHECK
                nph_source = resc['nph_source']*nspaxels
                self.logger.debug(np.max(nph_source.data)) ### COMMENT AFTER CHECK
    ...
    def snr_from_cube(self, ins, cube):
        ...
        dark_noise = np.sqrt(ins['dcurrent']*nb_voxels*obs['ndit']*obs['dit']/3600) #nvoxel is not relevant since it's 1, but should be only 2d if one of the spacial direction is the same of the spectral one
    ...
    def snr_from_spec(self, ins, spec):
        ...
        tel_eff_area = self.tel['area'] * (1 - ins.get('obscuration', 0)) # telescope effective area [poteva prenderla come ins['obscuration']
        a = (w*1.e-8/(H_cgs*C_cgs)) * (tel_eff_area*1.e4) * (ins_atm.data)
        Kt =  ins_ins * a
...
def sigma2vdisp(sigma, l0):
    """compute velocity dispersion in km/s from sigma in A 
...
# ! ! ! modified in order to get the convolution with the PSF
def sersic(self, ins, reff, n, ell=0, kreff=8, oversamp=1, uneven=1):
    ...
    # # # # # # # # # # # # # # #
    #for now we assume that the Sersic is not convolved with the PSF at
    #a single wavelength
    DEF_WAVE = 8000.0
    # # # # # # # # # # # # # # #

    samp = ins['spaxel_size'] # image sampling in arcsec
    ...
    # # # # # # # # # # # # # # #
    #IQ fwhm
    fwhm = get_seeing_fwhm(self.obs['seeing'], self.obs['airmass'], DEF_WAVE,
                               self.tel['diameter'], ins['iq_fwhm_tel'], ins['iq_fwhm_ins'])[0]
    pixfwhm = oversamp*fwhm/samp
    #to have the same WCS as the Moffat and get the PSF
    psf = moffat_image(fwhm=(pixfwhm,pixfwhm), n=ins['iq_beta'], shape=(ns,ns), flux=1.0, unit_fwhm=None)
    ...
    # # # # # # # # # # # # # # #
    # Convolve Sersic with PSF
    ima_conv = ima
    ima_conv.data = fftconvolve(ima.data, psf.data, mode='same')
    # # # # # # # # # # # # # # #

    ima_conv.data /= ima_conv.data.sum()

    # # # # # # # # # # # # # # #
    #we copy the WCS from the Moffat image
    ima.wcs = psf.wcs
    # # # # # # # # # # # # # # #
