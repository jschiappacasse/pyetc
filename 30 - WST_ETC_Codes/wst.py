...
#!!! modified every iq splitting equally the contribution of telescope and instrument (np.sqrt(2)/2)
...
class WST(ETC):
    ...
    def __init__(self, log=logging.INFO, skip_dataload=False):
        ...
        self.moslr = {} 
        self.moslr['channels'] = ['blue','orange', 'red']       
        # MOS-LR blue channel 
        ...
        self.moslr[chan] = dict(desc='Inspired from 4MOST LR throughput',
                                ...
                                iq_fwhm_tel = np.sqrt(2)/2 * 0.30, # fwhm PSF of telescope
                                iq_fwhm_ins = np.sqrt(2)/2 * 0.30, # fwhm PSF of instrument
                                iq_beta = 2.50, # beta PSF of telescope + instrument
                                spaxel_size = 0.25, # spaxel size in arcsec
    ...
    def info(self, ins=None, full=False):
        if ins is None:
            self._info(['ifs', 'moslr', 'moshr'], full=full)
        else:
            self._info([ins], full=full)
